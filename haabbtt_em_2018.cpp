...
int main() {
  ...
  cb.AddObservations({"*"}, {"haabbtt"}, {"2018"}, {"em"}, cats); // keep haabbtt for this analysis, change tt to the final state (eg em for emu). Same in the rest of the code

  // List of backgrounds in the datacards
  vector<string> bkg_procs = {"HTT","HWW","W","ZTT","TT","VV","Z","ST","ttHnonbb","QCD"};
  cb.AddProcesses({"*"}, {"haabbtt"}, {"2018"}, {"em"}, bkg_procs, cats, false);
  ...
  cb.AddProcesses(masses, {"haabbtt"}, {"2018"}, {"em"}, sig_procs, cats, true);
...
//  cb.cp().process({"Z"}).AddSyst(cb, "CMS_normalization_Z_misID_eletotauh_13TeV", "lnN", SystMap<>::init(1.2));
  ...
  TFile output("haabbtt2018_em.input.root", "RECREATE");
