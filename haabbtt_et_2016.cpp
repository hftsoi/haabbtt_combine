...
int main() {
  ...
  cb.AddObservations({"*"}, {"haabbtt"}, {"2016"}, {"et"}, cats); // keep haabbtt for this analysis, change tt to the final state (eg em for emu). Same in the rest of the code

  // List of backgrounds in the datacards
  vector<string> bkg_procs = {"HTT","HWW","ZTT","TT","VV","Z","ST","ttHnonbb","QCD"};
  cb.AddProcesses({"*"}, {"haabbtt"}, {"2016"}, {"et"}, bkg_procs, cats, false);

  // List of signals in the datacards. The mass will be concatenated at the end by the code
  vector<string> sig_procs = {"gghbbtt"};
  cb.AddProcesses(masses, {"haabbtt"}, {"2016"}, {"et"}, sig_procs, cats, true);
  ...
  cb.cp().process(ch::JoinStr({{"HTT","HWW","TT","VV","Z","ST","ttHnonbb"},sig_procs})).AddSyst(cb, "CMS_normalization_eletrg_13TeV", "lnN", SystMap<>::init(1.02));
  ...
  cb.cp().process({"ZTT"}).AddSyst(cb, "CMS_normalization_embedded_eletrg_13TeV", "lnN", SystMap<>::init(1.02));
  ...
  TFile output("haabbtt2016_et.input.root", "RECREATE");
