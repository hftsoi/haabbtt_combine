...
int main() {
  ...
  cb.AddObservations({"*"}, {"haabbtt"}, {"2016"}, {"mt"}, cats); // keep haabbtt for this analysis, change tt to the final state (eg em for emu). Same in the rest of the code

  // List of backgrounds in the datacards
  vector<string> bkg_procs = {"HTT","HWW","ZTT","TT","VV","Z","ST","ttHnonbb","QCD"};
  cb.AddProcesses({"*"}, {"haabbtt"}, {"2016"}, {"mt"}, bkg_procs, cats, false);
  ...
  cb.AddProcesses(masses, {"haabbtt"}, {"2016"}, {"mt"}, sig_procs, cats, true);
  ...
  cb.cp().process({"gghbbtt20","gghbbtt40","gghbbtt60"}).AddSyst(cb, "CMS_normaliztion_pdf_Higgs_gg_13TeV", "lnN", SystMap<>::init(1.032));
  cb.cp().process({"vbfbbtt20","vbfbbtt40","vbfbbtt60"}).AddSyst(cb, "CMS_normaliztion_pdf_Higgs_qq_13TeV", "lnN", SystMap<>::init(1.021));
  ...
  TFile output("haabbtt2016_mt.input.root", "RECREATE");
