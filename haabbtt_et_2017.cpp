...
int main() {
  ...
  cb.AddObservations({"*"}, {"haabbtt"}, {"2017"}, {"et"}, cats); // keep haabbtt for this analysis, change tt to the final state (eg em for emu). Same in the rest of the code

  // List of backgrounds in the datacards
  vector<string> bkg_procs = {"HTT","HWW","ZTT","TT","VV","Z","ST","ttHnonbb","QCD"};
  cb.AddProcesses({"*"}, {"haabbtt"}, {"2017"}, {"et"}, bkg_procs, cats, false);

  // List of signals in the datacards. The mass will be concatenated at the end by the code
  vector<string> sig_procs = {"gghbbtt"};
  cb.AddProcesses(masses, {"haabbtt"}, {"2017"}, {"et"}, sig_procs, cats, true);
  ...
  TFile output("haabbtt2017_et.input.root", "RECREATE");
