...
int main() {
  ...
  cb.AddObservations({"*"}, {"haabbtt"}, {"2018"}, {"et"}, cats); // keep haabbtt for this analysis, change tt to the final state (eg em for emu). Same in the rest of the code

  // List of backgrounds in the datacards
  vector<string> bkg_procs = {"HTT","HWW","ZTT","TT","VV","Z","ST","ttHnonbb","QCD"};
  cb.AddProcesses({"*"}, {"haabbtt"}, {"2018"}, {"et"}, bkg_procs, cats, false);
  ...
  cb.AddProcesses(masses, {"haabbtt"}, {"2018"}, {"et"}, sig_procs, cats, true);
  ...
  cb.cp().process({"ZTT"}).AddSyst(cb, "CMS_normalization_embedded_tautrg_13TeV", "lnN", SystMap<>::init(1.02));
  ...
  TFile output("haabbtt2018_et.input.root", "RECREATE");
