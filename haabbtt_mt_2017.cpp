...
int main() {
  ...
  cb.AddObservations({"*"}, {"haabbtt"}, {"2017"}, {"mt"}, cats); // keep haabbtt for this analysis, change tt to the final state (eg em for emu). Same in the rest of the code

  // List of backgrounds in the datacards
  vector<string> bkg_procs = {"HTT","HWW","ZTT","TT","VV","Z","ST","ttHnonbb","QCD"};
  cb.AddProcesses({"*"}, {"haabbtt"}, {"2017"}, {"mt"}, bkg_procs, cats, false);
  ...
  cb.AddProcesses(masses, {"haabbtt"}, {"2017"}, {"mt"}, sig_procs, cats, true);
  ...
  TFile output("haabbtt2017_mt.input.root", "RECREATE");
